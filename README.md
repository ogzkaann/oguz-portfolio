...
Project facts, status, stack, media, and links are centralized in `src/data/projects.ts`. Localized editorial content lives in `messages/en.json` and `messages/de.json`; newer project-specific EN/DE copy that must remain synchronized is kept in focused typed data modules such as `src/data/physical-ops-copy.ts` and `src/data/kurye-patronu-copy.ts`.
